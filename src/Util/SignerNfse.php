...
class SignerNfse
{
     ...
     * @throws \DOMException
     */
    public static function signNFSe($certificado, $password, $dadosXML): string
    {
        $xml = $dadosXML;
        ...
        if (openssl_pkcs12_read($certificado, $certs, $password) && extension_loaded('openssl')) {
            ...
            // Calcule o DigestValue com base nos dados do XML
            $dadosXML = str_replace('<assinar/>', '', $dadosXML);
            $dadosXML = str_replace('<assinarLote/>', '', $dadosXML);
            $xmlDigestValue = base64_encode(sha1($dadosXML, true));
            $digestMethod = $doc->createElement('DigestMethod');
            $digestMethod->setAttribute('Algorithm', 'http://www.w3.org/2000/09/xmldsig#sha1');
            $reference->appendChild($digestMethod);
            ...
            $signatureString = preg_replace('/>\s+</', '><', $signatureString);
            $signatureString = trim($signatureString);
            $xmlAssinado = str_replace('<assinar/>', $signatureString, $xml);
            ...
            if (openssl_pkcs12_read($certificado, $certs, $password) && extension_loaded('openssl')) {
                ...
                // Cria o elemento Reference
                $reference = $doc->createElement('Reference');
                $reference->setAttribute("URI", "L1000");
                ...
                // Calcule o DigestValue com base nos dados do XML
                $dadosXML2 = str_replace('<assinarLote/>', '', $xmlAssinado);
                $xmlDigestValue = base64_encode(sha1($dadosXML2, true));
                $digestMethod = $doc->createElement('DigestMethod');
                $digestMethod->setAttribute('Algorithm', 'http://www.w3.org/2000/09/xmldsig#sha1');
                $reference->appendChild($digestMethod);
                ...
                $xmlAssinado = str_replace('<assinarLote/>', $signatureString, $xmlAssinado);
